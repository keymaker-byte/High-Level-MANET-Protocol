...
//	                }
	                ...
	                while (wifiHandler.getConnectionState() == WifiConnectionState.DISCONNECTED)
	                {
...
//        }
	...
	public boolean sendUdpMessage(NetMessage message){
		try
		{
			byte[] lenght =  Tools.intToByteArray(message.getSize());
				...
				try
	            {
	                while (true)
	                {
	                    ...
	                    if (buffer.length > 4)
	                    {
	                    	...
	                    	int size = Tools.byteArrayToInt(arraySize);
