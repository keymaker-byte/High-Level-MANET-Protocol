...
namespace CommLayer 
{
    ...
    public class Communication : CommHandlerI
    {
        ...
        /// </summary>
        private System.Threading.Timer bag;
        ...
        public void startNetworkingHandler()
        {
            try
            {
                ...
                produceEvent(CommunicationEvent.NETINFORMATION, "COMMUNICATION: setting user IP");
                configuration.NetUser.Ip = netHandler.NetData.IpTcpListener;
                produceEvent(CommunicationEvent.NETINFORMATION, "COMMUNICATION: setting user ID");
                ...
                timer = new System.Threading.Timer(communicationTimer, null, 0, configuration.NetData.TimeIntervalTimer);
                ...
                bag = new System.Threading.Timer(communicationBag, null, 0, configuration.NetData.TimeIntervalTimer);
        ...
        public void stopNetworkingHandler()
        {
            if (Interlocked.CompareExchange(ref stopPoint, 1, 0) == 0)
            {
                ...
                try
                {
                    ...
                    bag.Change(Timeout.Infinite, Timeout.Infinite);
                    bag.Dispose();
                    try
                    {
                        bagThread.Abort();
                    ...
                    if (Interlocked.CompareExchange(ref bagWaitPoint, 0, 1) != 1)
                    {
                        ...
                        while (Interlocked.CompareExchange(ref bagPoint, -1, 0) != 0)
                        {
                            ...
                            if (timeOut > configuration.NetData.WaitForTimerClose)
                            {
                                produceEvent(CommunicationEvent.NETINFORMATION, "WARNING: impossible to wait for bag");
                                break;
                            }
                        }
                    }
                ...
                try
                {
                    produceEvent(CommunicationEvent.NETINFORMATION,
                        ...
                        " sent: " + router.NMessagesSent +
                        " confirmed: " + router.NMessagesConfirmed +
                        " destroyed: " + router.NMessagesDestroyed +
                ...
                {
                    produceEvent(CommunicationEvent.NETINFORMATION, "COMMUNICATION: init vars error " + e.Message);
                }
        ...
        {
            produceEvent(CommunicationEvent.NETINFORMATION, "COMMUNICATION: network error " + e.Message);
        ...
        private void communicationTimerInteration()
        {
            try
            {
                ...
                //controlHandler.netInformationHandler("processUdpMesages ... internalSendMessage");
        ...
        /// </summary>
        /// <param name="objectState">Parámetro</param>
        private void communicationBag(Object objectState)
        {
        ...
        {
            if (router.NotSentSize > configuration.NetData.StateCritical || router.FailedMessagesSize > configuration.NetData.StateCritical || netHandler.TcpMessageQueue.size() > configuration.NetData.StateCritical)
            {
                configuration.NetUser.State = CommunicationQuality.CRITICAL;
            }
            else if (router.NotSentSize > configuration.NetData.StateOverloaded || router.FailedMessagesSize > configuration.NetData.StateOverloaded || netHandler.TcpMessageQueue.size() > configuration.NetData.StateOverloaded)
            {
